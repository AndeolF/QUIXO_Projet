...
class QLearningAgent:
    ...
    def update_q_value(self, game, state, action, reward, next_state):
        ...
        max_q_next = max(
            [self.get_q_value(next_state, a) for a in next_available_actions],
            default=0.0,
        )
    ...
    def canonical_state_action(self, q_value):
        ...
        transformations = []
        for k in range(4):
            rotated_state, rotated_action = QLearningAgent.rotate_state_action(
                state, action, k
            )
        ...
        for axis in axes:
            reflected_state, reflected_action = QLearningAgent.reflect_state_action(
                state, action, axis
            )
            ...
            while not done:
                ...
                if done == False:
                    ...
                    rival.update_q_value(
                        game, state, rival_action, rival_reward, next_state
                    )
...
for i in range(20):
    ...
    q_tables, rival_q_tables = agent1.train(game, episodes=10000, rival=agent2)
    all_q_tables = (q_tables, rival_q_tables)
    merged_q_table = merge_q_tables(all_q_tables)
    save_q_table(
        q_table=merged_q_table,
        path="../agent_qlearning_save/q_table_agent_sans_multi.pkl",
    )
